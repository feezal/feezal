...
- **MQTT auto-discovery** — automatically detects devices published by zigbee2mqtt, ESPHome, RedMatic and compatible devices and bridges; one click pre-wires all topics and attributes. Devices that publish Home Assistant MQTT discovery are picked up automatically — for example [AI-on-the-edge-device](https://github.com/jomjol/AI-on-the-edge-device) meter readers: just enable *Home Assistant discovery* in the device's web UI and the meter appears in feezal's discovery picker

...
function findElements(dir) {
    ...
    const elems = list.filter(p => !blacklist.includes(p)).filter(p => {
...
module.exports = (logger) => {
    ...
    elements.map(el => el.bare).forEach(name => logger.info('found element ' + name));
    return fsPromises.writeFile(
        path.join(__dirname, '..', 'www', 'editor', 'feezal-elements.js'),
        elements.map(el => `import '${el.bare}';`).join('\n') + '\n\n' +
        'window.feezal.elements = ' + JSON.stringify(elements.filter(el => el.type === 'element').map(el => el.bare.replace(/^@[^/]+\//, '')), null, '  ') + ';\n\n' +
        'window.feezal.themes = ' + JSON.stringify(elements.filter(el => el.type === 'theme').map(el => el.bare.replace(/^@[^/]+\//, '')), null, '  ') + ';\n'

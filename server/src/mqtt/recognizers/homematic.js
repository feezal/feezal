...
/**
 * Homematic native recognizers (A30 — extracted from native-discovery.js).
 *
 * All 7 Homematic recognizers (climate / contact / cover / light / switch /
 * sensor / lock) plus their shared infrastructure: the hm topic/mode builders
 * (hmSet/hmStatus/hmParamset/buildHmModes), HmIP virtual-receiver run grouping
 * (hmipVirtualGroup), the channel-type allowlists, the JSON-Extended `hm`
 * metadata they consume, and the B65 `:0` maintenance correlation used to
 * resolve availability/battery at read time (indexMaintenance / resolveMaintSeg
 * / applyHmAvailability) and the B64 stale-run-leader filter (isStaleHmGroup).
 *
 * These cohere: they consume one payload model and share the grouping logic.
 * The file is SELF-CONTAINED — it owns the hm prefix + climate-staleness config
 * and its own `hmRecognizers` array (so isStaleHmGroup can search it), and needs
 * nothing back from native-discovery.js, keeping the import one-directional
 * (native-discovery.js → recognizers/*). See native-discovery.js for the
 * recognizer contract and the framework dispatch.
 */
...
function isStaleHmGroup(entity) {
    ...
    if (!g) return false;
    const rec = hmRecognizers.find(r => r.id === g.recognizerId);
...
// ── A30 accessors for the framework ───────────────────────────────────────────
/** Current climate-staleness window (ms) — the framework's read-time filter. */
function getClimateStaleMs() { return hmClimateStaleMs; }
/** Clear the :0 maintenance index (part of clearNativeEntities). */
function clearHmMaintenance() { hmMaintSeg.clear(); }

// Ordered HM recognizer array (climate first, then the rest) — isStaleHmGroup
// searches this; the framework interleaves wled between climate and contact to
// preserve the original registry order.
const hmRecognizers = [
    hmClimateRecognizer, hmContactRecognizer, hmCoverRecognizer,
    hmLightRecognizer, hmSwitchRecognizer, hmSensorRecognizer, hmLockRecognizer,
];

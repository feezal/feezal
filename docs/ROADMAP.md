...
- **Remaining:** the valve indicator + `valve-min/max` on `glass-climate`/`metro-climate` (a small visual add — material-climate has the scaling); the **profile-stamping presets** (5 bridge×generation templates) and the **boost remaining-time countdown** display stay gated on the open points below (real-device verification of the countdown datapoints / paramset-topic spelling / inter-publish delay) — the client-side countdown fallback and the model itself are unblocked and shipped.

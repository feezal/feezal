...
>
> **Fix:** `_dragRestriction()` in [feezal-sidebar-inspector.js](../www/src/feezal-sidebar-inspector.js) — per-axis branch: fixed-px axes clamp to the view's full layout size (`offsetWidth`/`offsetHeight`), auto axes get **no upper clamp** (canvas is unbounded; autoscroll follows). Plus the related snap-line fix: vertical guide lines are now translated into `#container-view`-relative coordinates (`viewLeftInCv`), so they sit on the true element edges on a scrolled canvas (the horizontal lines already used container-relative coords). Covered by browser tests (restriction geometry, scrolled guide placement) and e2e tests (`test-e2e/editor-drag-oversized.test.js`: drag past the old cutoff on a scrolled auto view — verified to FAIL against the old code —, fixed-view edge clamp + reachability).

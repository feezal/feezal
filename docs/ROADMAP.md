...
- **Remaining:** port the mode-descriptor model + valve (with `valve-min/max`) to `glass-climate`/`metro-climate` (each needs a valve indicator + the popup/tile mode UI); the **profile-stamping presets** (5 bridge×generation templates) and the **boost remaining-time countdown** display stay gated on the open points below (real-device verification of the countdown datapoints / paramset-topic spelling / inter-publish delay) — the client-side countdown fallback and the model itself are unblocked and shipped.

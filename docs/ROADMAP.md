...
**Homematic topic structure — confirmed (RedMatic / hm2mqtt defaults, 07/2026).** The recognizer's whole keying scheme is now pinned down (from the RedMatic default topic config):
- **Read/event:** `hm/status/${channelName}/${datapoint}` — `channelName` is a **single topic level**, so `hm/status/+/<DATAPOINT>` wildcard matching over the firehose is clean.
- **Write:** `hm/set/${channelNameOrAddress}/${datapoint}`.
- **putParamset:** `hm/paramset/${channelNameOrAddress}/${paramset}` (→ the `…/VALUES` combined write E102 uses); **putParam:** `hm/paramset/${channelNameOrAddress}/${paramset}/${param}`.
- **Prefix** `hm` is the default (configurable — make it a recognizer setting, but default `hm`).
- **Payload = MQTT-Smarthome "JSON Extended"** → the value is at **`.val`** (`{"val":…,"ts":…,"lc":…,"hm":…}`), NOT a bare payload. The recognizer must stamp **`message-property: val`** (and the boost-remaining / valve message-property variants) on every read attribute — this is also what the manual E102 path required.
- Status topics follow the **mqtt-smarthome retained convention** (believed retained → instant discovery on connect; confirm on the live system).

**Homematic-climate recognizer.** Watch a whitelist of thermostat datapoints on `hm/status/+/<DP>` (`SET_TEMPERATURE`, `SET_POINT_TEMPERATURE`, `CONTROL_MODE`, `SET_POINT_MODE`, `ACTUAL_TEMPERATURE`, `VALVE_STATE`, `LEVEL`, `BOOST_MODE`, `HUMIDITY`, …); accumulate the per-`channelName` datapoint set; promote a channel to a discovered `climate` entity once its **signature** matches (from E102's device matrix, preserved in ROADMAP-ARCHIVE): `SET_TEMPERATURE`+`CONTROL_MODE` → **BidCoS**, `SET_POINT_TEMPERATURE`+`SET_POINT_MODE` → **HmIP**; `VALVE_STATE`/`LEVEL` present → TRV (else wall/group). Assemble read/write/paramset topics from the observed `channelName` (`hm/status|set/<channelName>/<DP>`, `hm/paramset/<channelName>/VALUES`) and build the mode/publish/valve/putParamset wiring via `climate-profiles.js` — the generation templates already produce exactly these, so the recognizer is essentially "profile, but with the channelName + prefix observed instead of typed, and `message-property:val` set." Synthesize a `config` the climate elements' existing `discovery.map` consumes (the HA map keys like `temperature_state_topic` don't 1:1 match Homematic → likely a small native-config shape or a second `discovery.map` variant; TBD at implementation). Surface **per-thermostat-channel** entities (the channel carrying the setpoint), not every channel of a device.
...
**Open questions / verification (to settle at implementation):**
- **Homematic topic layout — RESOLVED** (see "Homematic topic structure" above: `hm/status|set/<channelName>/<DP>`, `hm/paramset/<channelName>/VALUES`, JSON-Extended `.val`, prefix `hm`). Remaining small confirmations: retained-ness on the live system (believed yes per mqtt-smarthome), and `channelName` character handling (CCU friendly names may contain spaces/umlauts — confirm RedMatic's topic sanitisation, since a `+` wildcard level tolerates most but the display/label should be readable). WLED retained-ness (`/v`, LWT) still believed yes — confirm.
- **Channel → device grouping** — a physical Homematic device has several channels (`:0` maintenance, `:4` thermostat, …); surface only the relevant thermostat channel. With channelName topics (not addresses) the channels of one device may not share an obvious prefix — grouping by device is best-effort; surfacing the thermostat channel directly is the MVP. The server already has a richer `/api/discovery/device-groups` endpoint (currently unused by the client) if grouping is wanted later.

...
### B8 — Elements cannot be dragged to the far edge of an oversized view 🧪 fix shipped 07/2026 — verify before archiving

> **Status 07/2026:** fix implemented and shipped after multiple earlier attempts had to be reverted. **Kept in the roadmap deliberately until verified on a real oversized dashboard** — see the checklist in TESTING.md §3 ("Oversized/scrolled canvas drags"). Rollback path: `git revert` of the B8 commits (isolated to `feezal-sidebar-inspector.js` + tests/docs).
>

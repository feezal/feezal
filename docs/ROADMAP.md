...
**Progress (07/2026):** the fully-decided **valve scaling** slice landed for **material-climate** — `valve-min`/`valve-max` attributes scale an incoming valve level from the device range to 0–100 % (defaults 0/100 = BidCoS `VALVE_STATE` unchanged; set `valve-max=1` for HmIP `LEVEL` 0…1), applied to both separate-topic and JSON-payload valve reads, with a "Valve range" section + ℹ help in the Config inspector (browser-tested). **Remaining valve work:** bring `subscribe-valve` + the same scaling to `glass-climate`/`metro-climate` (each needs a valve indicator in its own visual language). **The mode-descriptor / boost / profile-stamping core stays gated — do not implement** until U39 (inspector restructuring) lands *and* the datapoints/topics are verified against real devices (open points below).
...
- **Per-entry mode descriptors** replacing the flat `modes` value/label list. Each entry: `{value, label, publish?, payload?}` — `value` matches the *read* topic's payload (existing `subscribe-mode`), `publish`/`payload` override the default `publish-mode` write. `payload` may be a string **or a JSON object** (published as-is — this is how the putParamset topic gets its multi-param payload), and supports a **`$setpoint` placeholder** (also inside object values) resolving to the remembered last-real setpoint (covers `MANU_MODE = <temp>`) and constants like `4.5` (covers "off"). Example (BidCoS TRV + HmIP-off via putParamset, hm2mqtt-style topics):
  ...
  ```
  **Combined mode+setpoint writes go through the bridge's putParamset topic** (`hm/paramset/<channel>/VALUES`, finding 5) — one atomic publish, no ordering/timing concerns. For bridges *without* a paramset topic, an entry may instead carry an ordered **`steps` array of `{topic, payload}`** published in sequence (mode first, then setpoint) — kept as the generic fallback mechanism only. (Exact shape of the off-entry's display-match rule — `match-setpoint-max` or similar — and the `steps` field syntax TBD at implementation time; entries without `publish`/`steps` keep today's behaviour, so the change is **backwards compatible**.)
...
2. Exact JSON shape of mode entries (`steps` field syntax, `match-setpoint-max` naming, `momentary`/`off` fields, object-valued `payload`) — settle when the U39 inspector work defines how these entries are edited.
3. Confirm the bridge's exact paramset-topic spelling against the live system (`hm/paramset/<channelNameOrAddress>/VALUES` — verify the segment is `VALUES` (plural, as in the XML-RPC API and RedMatic) rather than `VALUE`, and the exact JSON payload shape the bridge expects) before hard-coding it into the profile presets.
4. Inter-publish delay for multi-step writes — **only relevant for the `steps` fallback** on bridges without a paramset topic (the hm2mqtt profiles use the atomic putParamset publish and are unaffected). RedMatic staggers *linked-device* writes by 3 s; check whether sequential datapoint writes to the *same* device need any delay over MQTT or can fire back-to-back.

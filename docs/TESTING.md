...
- [ ] **Climate valve scaling (E102, partial)** (material-climate): with `subscribe-valve` set and the defaults (valve-min 0 / valve-max 100), a Homematic BidCoS `VALVE_STATE` of `73` shows a 73 % valve arc/bar unchanged; set **valve-max = 1** (Config → Valve range) for an HmIP `LEVEL` device → an incoming `0.5` shows 50 %, `1` shows 100 %; out-of-range values clamp to 0–100 %; the same scaling applies when the valve is read from a JSON payload (`json-map` `valve` key). Config tab shows the Valve range Min/Max fields with the HmIP ℹ hint. *(Separate + JSON scaling, clamping and the zero-span guard are browser-tested.)* **Deferred (do not test yet):** mode-descriptor/boost/profile-stamping core and glass/metro valve — gated on U39 + real-device verification.

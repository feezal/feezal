...
- [ ] button · [ ] sensor · [ ] light · [ ] switch · [ ] climate · [ ] contact · [ ] shutter · [ ] occupancy · [ ] fan · [ ] dialog · [ ] dialog-view · [ ] countdown-dialog · [ ] wled
...
- [ ] **Glass wide-tile row layout (E105)** (glass button/switch/contact/sensor/light/shutter/occupancy/climate/fan): resize any card to at least twice as wide as tall → the layout flips to icon LEFT with state/label stacked right of it (Apple-Home wide-tile style), live while resizing; back under 2:1 → the vertical stack returns; overlay chrome (⋯/tune, ⚠ badge) keeps its corners; popups unaffected; check a very flat card (e.g. 320×80) has no overflow. Family-wide 2:1 breakpoint — mixed dashboards flip consistently. *(Grid/flex switching is browser-tested for three cards.)*

...
- [ ] **Background editor / style groups (N34)**: select a **view** → the Styles tab shows a **Background** widget (mode buttons None / Solid / Image / Gradient) instead of a raw `background` row, and `width`/`height` stay ordinary rows. **Solid**: colour swatch writes `background-color`. **Image**: URL field (or use the asset manager's *Set as background*, N33 — the editor reflects the stamped values on re-select), size select (cover/contain/auto/custom + free input), repeat select, 3×3 position anchor grid + custom field — each control updates the canvas immediately. **Gradient**: linear (angle °) / radial (circle/ellipse), ordered colour stops (add / ↑↓ reorder / ✕ remove, min 2), live preview bar; the serialised `linear-gradient(…)`/`radial-gradient(…)` lands in `background-image`. **None** clears the whole family. Re-selecting the view round-trips every mode back into the editor's fields; only longhands are written (check source mode — no `background` shorthand; a legacy inline shorthand is migrated to longhands on first edit). The covered `background-*` longhands never appear as extra custom rows and "Add CSS property" refuses them; undo after an edit reverts the whole multi-property change in one step. *(Mechanism — group items, covers, suppression, mount, multi-property apply incl. null-removal, add-prop refusal — and the editor's mode detection, shorthand fallback, gradient round-trip and stop editing are unit-tested.)*

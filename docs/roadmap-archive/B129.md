...
position. `entry-style="list"` gap and the edge-rail padding zero moved to
the scroller with their values unchanged.

*Correction (same day):* the full browser run caught what the layout-app
suite alone had not — the B90 geometry pins (`layout-app-drawer-geometry` /
`-rail-drawer`) flagged two follow-ups: the rail expansion rules still
re-set `padding` on the now-unpadded `.drawer` shell (double inset on
hover-expand; the edge rail's padding restore now targets `.drawer-nav`),
and the rail-menu hamburger belongs INSIDE the padded scroller after all —
its position is part of the pinned geometry. Both fixed with the U109
commit; the U94 scrollbar tests and the inspector padding assertion moved
to `.drawer-nav` with the scroller.

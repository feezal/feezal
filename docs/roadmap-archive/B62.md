...
## ✅ Actually fixed (07/2026, third attempt) — measured, not reasoned

**Everything below this section is superseded.** Two fixes shipped green and changed nothing, because *every one of them re-plumbed a layer that does not paint the pixels in the report*.

**Measured root cause** (E2E, real Chromium, `www/test-e2e/b62-sticky-background.test.js`). In a 420×720 viewport, a `width/height: 100%` flow view with 40 tiles:

| box | its own height | its content height | background |
|---|---|---|---|
| `feezal-site` (the only scroller) | 720 | 2295 | *(none — cleared by attempt #1)* |
| `feezal-view` | **720** | 2295 | **the gradient**, `attachment: scroll` |

The gradient is painted by **`feezal-view`**, whose box is exactly **one viewport** while its flow content is **three viewports** tall. `overflow` is `visible`, so the content spills outside the box that paints the background; scrolling drags that single band of gradient away and reveals whatever sits behind it. Nothing about iOS, `background-attachment`, compositing or the `html`/`body` mirror — it reproduces in desktop Chromium, and the original "desktop is correct" finding was simply wrong.

**Fix.** The scroller paints the gradient; the view is told not to.
- `feezal-site` — for `[gradient-bg]`, paint with `background-attachment: scroll` (the default), `cover`, `no-repeat`. This host *is* one viewport, and `scroll` anchors a background to the element's own box rather than to the scrolled content, so the result is viewport-pinned **without `position: fixed` and without `background-attachment: fixed`** — the property iOS does not honour. The problem that defeated two attempts dissolves rather than being worked around.
- `:host(.feezal-viewer[gradient-bg]) ::slotted(feezal-view) { background-image: none !important }` — kills the band. `!important` is required because it overrides an *inline* style written by the background style editor.
- `layout-app._embed()` — the same defect on a second path: it clones the view into its shadow root **and** copies the background onto `.content`, so both paint. The clone's `background-image` is now cleared and `.content` gets `scroll`/`cover`/`no-repeat`.

**Behaviour chosen deliberately** (the reporter picked it from two options): the background is **pinned to the viewport** and content scrolls over it — not "grows with the content".

**Tests.** `www/test-e2e/b62-sticky-background.test.js` samples the **rendered pixels** at a fixed viewport coordinate, scrolls, and samples again; pinned means byte-identical PNGs. Verified red before each change and green after, separately for the plain path and the layout-app path. This is deliberate penance for attempt #2, whose tests asserted that the stylesheet *text* contained `position: fixed` — a test that passes whether or not the layout works, and did.

**Still open, deliberately NOT claimed as fixed here:**
- **The `backdrop-filter` artifacts** on glass cards. The reporter's check isolated them: same gradient with non-glass cards → no artifacts. Distinct bug, distinct fix.
- **A second scrollbar** seen in desktop Chrome. Not reproduced in either E2E configuration (`document.scrollsBy: 0` in both) — needs its own repro before anyone touches it.

**Also explains [U61](../ROADMAP.md).** The editor's checkerboard showing in the scroll overflow is the *same* overflow: the area beyond the view's box was never painted by the view. Same root cause, seen from the editor side.

---

**✅ Fixed (07/2026) — attempt #1, superseded; did not fix the reported problem.** In `www/src/feezal-site.js` (+ `www/test/feezal-site.test.js`).

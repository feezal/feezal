...
>
> **Trigger & persistence:** auto-starts only when the seen-flag is unset AND no view carries any element (fresh install); Skip/Done set the flag. Re-launchable from Editor Settings → Help → "Show welcome tour". Flag is localStorage — the server-side variant was deferred: the empty-site condition already prevents re-showing on existing dashboards from other browsers, so the flag only matters for fresh installs. Unit tests (trigger gating) + browser tests (step machinery, sidebar switching, click-blocking, hands-on advance, persistence).

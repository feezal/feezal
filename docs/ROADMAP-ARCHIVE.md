...
**Deferred / needs live-system confirmation:** (a) a **real-broker smoke test** that discovery actually finds thermostats (retained-ness of `hm/status` topics — believed yes per mqtt-smarthome; `channelName` character handling for readable labels); (b) **Homematic availability** auto-wiring (the `:0` maintenance `UNREACH` channel) — HM entities ship without `availability_normalized`, so availability is configured manually for now; (c) optional `subscribe-boost-remaining` device-datapoint wiring (`BOOST_STATE`/`BOOST_TIME` names/units) — the client boost countdown works regardless; (d) channel→device grouping (per-thermostat-channel entities are surfaced today).

 ...
 *
 * Frosted-glass alarm-sensor card. A VIEW over SensorController, narrowed to
 * the ALARM vocabulary slice: the `type` variants (water-leak / smoke / gas /
 * co / vibration / tamper / generic) pick the default hazard icon and text;
 * the card highlights in the error colour while triggered. Same MQTT capability
 * contract as the Circle sensor card — attribute names, payload matching incl.
 * JSON {state} and boolean coercion, availability badge, E124 low-battery badge
 * and the HA `binary_sensor` discovery descriptor incl. `device_class`.
 *
 * E138: the active-state accent DEFAULTS from the controller's slice colour var
 * (`activeColorVar()` → `--error-color` for the alarm slice); the per-element
 * `--feezal-glass-active-color` style override still wins.
 *
...
class FeezalElementGlassSensor extends FeezalElement {
    static get feezal() {
        return {
            // E138: ALARM slice — the hazard-sensor card.
            palette: {name: 'Sensor', category: 'Glass', color: '#7aa5c9', icon: 'warning'},
            description: 'Frosted-glass alarm-sensor card (water leak, smoke, gas, CO, vibration, tamper, …) — ' +
                'error-coloured while triggered. Same MQTT contract as the Circle sensor card.',
            // E137/E138: discovery map is the controller package's alarm-slice fragment.
            ...
            styles: [
                ...
                {property: '--feezal-glass-active-color', type: 'color', default: 'var(--error-color)', help: 'Icon/state colour while triggered. Defaults to the theme error colour.'},
    ...
    static styles = [feezalBatteryStyles, feezalFaultStyles, feezalBaseStyles, glassCardStyles, css`
        ...
        /* E138: active accent — per-element --feezal-glass-active-color override
           ...
           via --feezal-glass-active-default; alarm slice → --error-color). */
        .card.active {
            background: var(--feezal-glass-on-tint, rgba(255,255,255,0.62));
            --_state-color: var(--feezal-glass-active-color, var(--feezal-glass-active-default, #d32f2f));
        }
    ...
    constructor() {
        ...
        // E138: alarm slice defaults to the 'generic' hazard type.
        this.type = 'generic';
        this.icon = '';
        this.iconActive = '';
        this.iconClear = '';
    ...
    render() {
        ...
        // var (alarm → --error-color); the per-element style override wins.

 ...
 *
 * E-ink MOTION card (motion / occupancy / presence / radar): icon +
 * oversized state word, whole card INVERTS while active (the 1-bit
 * "attention" treatment). Alarm-character sensors live in eink-sensor.
...
class FeezalElementEinkMotion extends EinkBase {
    static get feezal() {
        return {
            palette: {name: 'Motion', category: 'Eink', color: '#222222', icon: 'sensors'},
            description: 'E-ink motion card (motion / occupancy / presence / radar) — inverted block while active, 1-bit, redraw-deduped.',
            discovery: {component: 'binary_sensor', map: sensorDiscoveryMapFor('motion')},
            attributes: [
                // E137/E138: the shared sensor contract, MOTION slice of the
    ...
    constructor() {
        super();
        this.type = 'motion'; // E138: motion-slice default type
        this.subscribe = '';
        this.msgProp = '';
...
customElements.define('feezal-element-eink-motion', FeezalElementEinkMotion);
export {FeezalElementEinkMotion};

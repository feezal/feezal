...
class FeezalElementMaterialThermostat extends FeezalElement {
    static get feezal() {
        return {
            ...
            // zigbee2mqtt climate devices emit a schema:json discovery config
            // pointing at a single base topic with a consolidated JSON object.
            // The element defaults payload-mode to 'separate' for hand-wiring
            // back-compat; discovery sets it to 'json'.
            discovery: {
                component: 'climate',
                map: {
                    // JSON wiring (discovery uses schema:json by default)
                    state_topic:                {attr: 'subscribe'},
                    temperature_command_topic:  {attr: 'publish', onlyWhen: {schema: 'json'}},
                    // Separate wiring fallback
                    ...
                    max_temp:                   {attr: 'max'},
                    temp_step:                  {attr: 'step'},
                    temperature_unit:           {attr: 'unit', transform: 'prefix:°'},
                    // Modes list → JSON array string
                    modes:                      {attr: 'modes', transform: 'join'},
                    // Availability + label
    ...
    static styles = [feezalBaseStyles, css`
        :host {
            ...
            --feezal-thermostat-heat-color:  var(--accent-color,         var(--feezal-accent,  #ff7043));
            --feezal-thermostat-cool-color:  var(--info-color,           var(--feezal-info,    #29b6f6));
            --feezal-thermostat-idle-color:  var(--secondary-text-color, var(--feezal-color2,  #aaa));
            --feezal-thermostat-text-color:  var(--primary-text-color,   var(--feezal-color,   #212121));
            --feezal-thermostat-error-color: var(--error-color, #b00020);

            /* MD3 bridge — filter-chips follow the thermostat theme */
            --md-sys-color-on-surface-variant: var(--feezal-thermostat-idle-color);
            --md-filter-chip-outline-color:    var(--feezal-thermostat-idle-color);
            --md-filter-chip-label-text-color: var(--feezal-thermostat-text-color);
            --md-filter-chip-selected-container-color:   var(--feezal-thermostat-heat-color);
            --md-filter-chip-selected-label-text-color:  #fff;
            --md-sys-color-primary: var(--feezal-thermostat-heat-color);
        }
        .unavail {
            position: absolute;
            top: 6px; right: 6px;
            width: 18px; height: 18px;
            color: var(--feezal-thermostat-error-color);
        ...
        .valve-label {
            ...
            opacity: 0.6;
            white-space: nowrap;
            color: var(--feezal-thermostat-text-color);
        }
        .valve-bar {
            flex: 1;
            height: 4px;
            background: var(--feezal-thermostat-idle-color);
            border-radius: 2px;
            overflow: hidden;
        }
        .valve-fill {
            height: 100%;
            border-radius: 2px;
            background: var(--feezal-thermostat-heat-color);
    ...
    get _arcFillColor() {
        ...
        if (!mode || mode === 'heat' || mode === 'emergency_heat') return 'var(--feezal-thermostat-heat-color)';
        if (mode === 'cool') return 'var(--feezal-thermostat-cool-color)';
        if (mode === 'off') return 'var(--feezal-thermostat-idle-color)';
        return 'var(--feezal-thermostat-heat-color)'; // auto, fan_only, etc.
    }
    ...
    _renderArc() {
        ...
        if (actualSpan !== null) {
            ...
            actualTick = svg`<line x1="${tx}" y1="${ty}" x2="${tx2}" y2="${ty2}"
                stroke="var(--feezal-thermostat-text-color)" stroke-width="2"
        ...
        if (this._valve !== null) {
            ...
            valveArc = svg`
                <path d="${tArcPath(ARC_START, ARC_SPAN, VALVE_R)}" fill="none"
                    stroke="var(--feezal-thermostat-idle-color)" stroke-opacity="0.3"
        ...
        return svg`
            ...
            <path d="${bgPath}" fill="none"
                stroke="var(--feezal-thermostat-idle-color)" stroke-opacity="0.25"
            ...
            <circle cx="${hx}" cy="${hy}" r="8"
                fill="${fillColor}" stroke="var(--feezal-thermostat-text-color)"
            ...
            <text x="${ARC_CX}" y="${ARC_CY + 8}"
                ...
                fill="var(--feezal-thermostat-text-color)"
            ...
            ${dispSetpoint ? svg`
                <text x="${ARC_CX}" y="${ARC_CY + 30}"
                    text-anchor="middle" dominant-baseline="middle"
                    font-size="11" fill="var(--feezal-thermostat-text-color)"
    ...
    _parsedModes() {
        ...
        try { return JSON.parse(this.modes); } catch { return []; }
    }
...
customElements.define('feezal-element-material-thermostat', FeezalElementMaterialThermostat);
export {FeezalElementMaterialThermostat};
...
class FeezalElementMaterialThermostatInspector extends LitElement {
    static properties = {
...
customElements.define('feezal-element-material-thermostat-inspector', FeezalElementMaterialThermostatInspector);
export {FeezalElementMaterialThermostatInspector};

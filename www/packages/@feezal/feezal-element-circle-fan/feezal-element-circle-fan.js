...
// ── Element ───────────────────────────────────────────────────────────────────
class FeezalElementCircleFan extends FeezalElement {
    static get feezal() {
        return {
            palette: {name: 'Fan', category: 'Circle', color: '#1565c0', icon: 'mode_fan'},
    ...
    static styles = [feezalBaseStyles, css`
        :host {
            ...
            gap: 4px;
            position: relative;
            /* E139: the disc ring sizes in cqi (card width); currentColor anchors
        ...
        /* E139: the fan blades sit inside the Circle-family disc — width-sized,
           top-anchored, concentric with the light ring; the ring tints to the
           accent colour while the fan is ON. Controls stack below. */
        .disc-wrap {
        ...
        .disc.on {
            ...
            background: color-mix(in srgb, var(--feezal-fan-accent-color) 10%, transparent);
        }
        svg.fan { width: 62%; height: 62%; display: block; overflow: visible; }
        .toggle-row {
...
customElements.define('feezal-element-circle-fan', FeezalElementCircleFan);
export {FeezalElementCircleFan};

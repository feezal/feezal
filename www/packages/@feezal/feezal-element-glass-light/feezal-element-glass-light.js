    ...
    static get feezal() {
        return {
            ...
            attributes: [
                ...
                {name: 'label-on',  type: 'string', default: 'On', defaultI18n: {de: 'Ein'},  help: 'Displayed state text while the light is on (localise, e.g. "Ein"); the brightness suffix "• x %" keeps appending. Display only — NOT the MQTT payload (payload-on) and NOT the card title (label).'},
                {name: 'label-off', type: 'string', default: 'Off', defaultI18n: {de: 'Aus'}, help: 'Displayed state text while the light is off (localise, e.g. "Aus"). Display only — NOT the MQTT payload (payload-off) and NOT the card title (label).'},
            ...
            styles: [
                ...
                // E140: per-state icon colour — the OFF side (was the unexposed
                // frost-muted) is now overridable.

...
export const climateAttributes = [
    ...
    {name: 'valve-min', type: 'number', default: 0, help: 'Valve device range minimum (BidCoS VALVE_STATE 0–100 → leave default; HmIP LEVEL 0…1 → set valve-max to 1).'},
    {name: 'valve-max', type: 'number', default: 100, help: 'Valve device range maximum (100 for BidCoS, 1 for HmIP LEVEL; stamped from real metadata by discovery).'},
    ...
    {name: 'boost-duration', type: 'number', default: 5,
        help: 'Boost countdown: duration in minutes (Homematic default 5). Without subscribe-boost-remaining a client-side mm:ss countdown starts from this value.'},
    {name: 'subscribe-boost-remaining', type: 'mqttTopic',
        help: 'Boost countdown: optional device-reported remaining time (badge shows this instead of the client timer).'},
    ...
    {name: 'subscribe-boost-state', type: 'mqttTopic',
        help: 'Optional device-reported boost active state (Homematic BOOST_MODE). While truthy the boost entry shows active regardless of the mode read-back.'},

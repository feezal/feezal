...
/**
 * feezal-element-glass-dialog-view (E101)
 *
 * Frosted-glass sibling of feezal-element-material-dialog-view — SAME
 * attribute contract (view embedding, topics, payloads, ok/cancel, B24
 * sizing, B25 header behaviour), restyled with the glass family chrome:
 * translucent blurred panel, squircle corners, dimmed backdrop.
 *
 ...
 *
 * Panel sizing/chrome is themeable via --feezal-glass-dialog-view-* custom
 ...
 * property. Family conventions (frost vars, degrade, squircle): glass-button.
 */
...
// Glass chrome vars mirrored the same way (theme :root vars reach the portal
...
class FeezalElementGlassDialogView extends FeezalElement {
    static get feezal() {
        return {
            palette: {name: 'Dialog View', category: 'Glass', color: '#7aa5c9'},
            description: 'Frosted-glass modal dialog showing a feezal view (with its live elements) when an ' +
                'MQTT message is received. A view-based alternative to the template-body glass Dialog element.',
            attributes: [
                ...
                {name: 'width',             type: 'string',    default: '',       help: 'Convenience: sets --feezal-glass-dialog-view-width (e.g. "600px").'},
                {name: 'height',            type: 'string',    default: '',       help: 'Convenience: sets --feezal-glass-dialog-view-height (e.g. "400px"). Empty: auto (content height).'},
                {name: 'min-height',        type: 'string',    default: '',       help: 'Convenience: sets --feezal-glass-dialog-view-min-height (e.g. "300px") — raises the floor so a dialog with little content does not collapse.'},
                {name: 'max-height',        type: 'string',    default: '',       help: 'Convenience: sets --feezal-glass-dialog-view-max-height (e.g. "85vh").'},
            ...
            styles: [
                'top', 'left',
                {property: '--feezal-glass-accent', type: 'color', default: '#ff9f0a', help: 'OK button accent colour.'},
            ...
            inspector: 'feezal-element-glass-dialog-view-inspector',
        };
    }
    ...
    static styles = [feezalBaseStyles, css`
        :host {
            ...
            /* ── Exposed panel sizing / chrome ── */
            --feezal-glass-dialog-view-width: 600px;
            --feezal-glass-dialog-view-height: auto;
            --feezal-glass-dialog-view-min-height: auto;
            --feezal-glass-dialog-view-max-width: calc(100vw - 32px);
            --feezal-glass-dialog-view-max-height: 85vh;
            --feezal-glass-dialog-view-radius: var(--feezal-glass-radius, 24px);
            --feezal-glass-dialog-view-padding: 0px;
            --feezal-glass-dialog-view-background: var(--feezal-glass-tint, rgba(255,255,255,0.7));
            --feezal-glass-dialog-view-backdrop: rgba(0,0,0,0.35);
        }
        ...
        .backdrop {
            position: fixed;
            inset: 0;
            background: var(--feezal-glass-dialog-view-backdrop, rgba(0,0,0,0.35));
            z-index: 9998;
        }
        .dialog-panel {
            position: fixed;
            ...
            z-index: 9999;
            width: var(--feezal-glass-dialog-view-width, 600px);
            height: var(--feezal-glass-dialog-view-height, auto);
            min-height: var(--feezal-glass-dialog-view-min-height, auto);
            max-width: var(--feezal-glass-dialog-view-max-width, calc(100vw - 32px));
            max-height: var(--feezal-glass-dialog-view-max-height, 85vh);
            background: var(--feezal-glass-dialog-view-background, rgba(255,255,255,0.7));
            ...
            border-radius: var(--feezal-glass-dialog-view-radius, 24px);
        ...
        .dialog-body {
            flex: 1;
            overflow: auto;
            padding: var(--feezal-glass-dialog-view-padding, 0px);
            position: relative;
        }
        .preview-note {
            width: 100%;
            ...
            opacity: 0.85;
            background-image:
                linear-gradient(45deg, rgba(0,0,0,0.06) 25%, transparent 25%, transparent 75%, rgba(0,0,0,0.06) 75%, rgba(0,0,0,0.06) 100%),
                linear-gradient(45deg, rgba(0,0,0,0.06) 25%, transparent 25%, transparent 75%, rgba(0,0,0,0.06) 75%, rgba(0,0,0,0.06) 100%);
    ...
    /** Mirror the host's --feezal-glass-dialog-view-* and --feezal-glass-*
    ...
    _syncTokens() {
        ...
        for (const t of SIZE_TOKENS) {
            const v = cs.getPropertyValue(`--feezal-glass-dialog-view-${t}`).trim();
            if (v) this._portal.style.setProperty(`--feezal-glass-dialog-view-${t}`, v);
        }
        ...
        if (this.width) this._portal.style.setProperty('--feezal-glass-dialog-view-width', this.width);
        if (this.height) this._portal.style.setProperty('--feezal-glass-dialog-view-height', this.height);
        if (this.minHeight) this._portal.style.setProperty('--feezal-glass-dialog-view-min-height', this.minHeight);
        if (this.maxHeight) this._portal.style.setProperty('--feezal-glass-dialog-view-max-height', this.maxHeight);
    ...
    _openPortal() {
        if (!this._portal) {
            ...
            this._portal.setAttribute('feezal-glass-dialog-view-portal', '');
        }
    ...
    _renderPortalContent() {
        return html`
            <style>
                [feezal-glass-dialog-view-portal] * { box-sizing: border-box; }
                [feezal-glass-dialog-view-portal] .backdrop {
                    position: fixed; inset: 0;
                    background: var(--feezal-glass-dialog-view-backdrop, rgba(0,0,0,0.35)); z-index: 9998;
                }
                [feezal-glass-dialog-view-portal] .dialog-panel {
                    ...
                    max-width: var(--feezal-glass-dialog-view-max-width, calc(100vw - 32px));
                    max-height: var(--feezal-glass-dialog-view-max-height, 85vh);
                    background: var(--feezal-glass-dialog-view-background, var(--feezal-glass-tint, rgba(255,255,255,0.7)));
                    -webkit-backdrop-filter: blur(var(--feezal-glass-blur, 20px));
                    backdrop-filter: blur(var(--feezal-glass-blur, 20px));
                    border: 1px solid var(--feezal-glass-border, rgba(255,255,255,0.55));
                    color: var(--feezal-glass-color, #1d1d1f);
                    ...
                    font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;
                    animation: feezal-glass-dialog-view-in 0.18s ease;
                }
                @supports (corner-shape: squircle) {
                    [feezal-glass-dialog-view-portal] .dialog-panel { corner-shape: squircle; }
                }
                [feezal-glass-dialog-view-portal][degrade] .dialog-panel {
                    -webkit-backdrop-filter: none; backdrop-filter: none;
                    background: var(--feezal-glass-solid, rgba(245,245,247,0.94));
                }
                @keyframes feezal-glass-dialog-view-in {
                    from { opacity: 0; transform: translate(-50%, -48%); }
                    to   { opacity: 1; transform: translate(-50%, -50%); }
                }
                [feezal-glass-dialog-view-portal] .dialog-header {
                    display: flex; align-items: center; gap: 8px;
                    padding: 14px 16px 10px; font-size: 16px; font-weight: 700;
                    color: var(--feezal-glass-color, #1d1d1f);
                }
                [feezal-glass-dialog-view-portal] .dialog-header .spacer { flex: 1; }
                [feezal-glass-dialog-view-portal] .dialog-close {
                ...
                [feezal-glass-dialog-view-portal] .dialog-close:hover {
                    background: color-mix(in srgb, var(--feezal-glass-color, #1d1d1f) 8%, transparent);
                }
                [feezal-glass-dialog-view-portal] .dialog-body {
                    flex: 1; overflow: auto; padding: var(--feezal-glass-dialog-view-padding, 0px); position: relative;
                }
                [feezal-glass-dialog-view-portal] .preview-note {
                    width: 100%; min-height: 120px; display: flex; align-items: center; justify-content: center;
                    font-size: 13px; color: var(--feezal-glass-muted, rgba(29,29,31,0.55));
                    padding: 16px; text-align: center;
                }
                [feezal-glass-dialog-view-portal] .dialog-footer {
                    padding: 8px 16px 14px; display: flex; justify-content: flex-end; gap: 8px;
                }
                [feezal-glass-dialog-view-portal] .dialog-btn {
                ...
                [feezal-glass-dialog-view-portal] .dialog-btn:hover { filter: brightness(0.9); }
                [feezal-glass-dialog-view-portal] .dialog-btn-ok {
                    background: var(--feezal-glass-accent, #ff9f0a); border-color: transparent; color: #fff;
                }
                [feezal-glass-dialog-view-portal] .dialog-btn-cancel {
                    background: color-mix(in srgb, var(--feezal-glass-color, #1d1d1f) 10%, transparent);
                    color: var(--feezal-glass-color, #1d1d1f);
                }
            </style>
...
customElements.define('feezal-element-glass-dialog-view', FeezalElementGlassDialogView);

// ── Custom inspector ───────────────────────────────────────────────────────────

class FeezalElementGlassDialogViewInspector extends FeezalElement {
    static properties = {
    ...
    static styles = css`
        ...
        .preview-btn {
            ...
            background: #7aa5c9; color: #fff; border: none; border-radius: 4px;
...
customElements.define('feezal-element-glass-dialog-view-inspector', FeezalElementGlassDialogViewInspector);
export {FeezalElementGlassDialogView, FeezalElementGlassDialogViewInspector};
